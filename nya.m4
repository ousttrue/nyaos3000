...
特徴として次のようなものがあります。
    ...
    ・逆クォートによる、他コマンド出力の引用機能
    ・横スクロールにより、画面幅を超える文字数もコマンドも入力可能
...
ifelse(SHELL,NYADOS,[[dnl
    errorlevel 節は option +standalone が設定されているときのみ
    正常に利用可能です。
]])dnl
...
------
    ...
    オプションには今のところ次のようなものがあります。
        ...
        dots
            設定すると、「...」→「..\..」,「....」→「..\..\..」
            といった置換を行います。DOS や OS/2 向けの機能です。
...
ifelse(SHELL,NYACUS,[[dnl
        ...
        nyatype
            起動時に自動的に設定します。NYADOS , NYACUS , NYAOS2 の
            いずれかが設定されます。
        history
...
------
    ...
    > source 設定ファイル
        ...
        なお、今のところ if errorlevel が解釈できません。
        (if exist , if == は解釈します)
...
また、次のような演算子も使用可能です。
    ...
    (注意)
        standalone オプションがセットされていない場合、PATH などの
        長い文字列(256文字以上)を含む変数を参照すると、CMD.EXE ,
        COMMAND.COM レベルでエラーになる場合があります。
        これは「option +standalone」を実行して、単独実行するよう
        にすると回避できます。
...
による画面制御はできませんが、NYACUS 内の内蔵コマンドとプロンプト
に限っては、NYACUS 自体がエミュレーションを行い、次のコードを解釈
します。
